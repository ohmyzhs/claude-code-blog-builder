...
> 🔁 **재개 모드**: 요청 맨 앞에 `[RESUME: output/<폴더>]` 표시가 있으면, 그 폴더에서 진행하다 중단된 글을 **이어서** 완성하는 모드입니다. 이때:
> 1. **새 폴더를 만들지 말고** 지정된 폴더를 그대로 사용합니다. STEP 1의 리서치 폴더 경로도 이 폴더로 고정하세요.
> 2. 먼저 해당 폴더의 기존 산출물을 Read로 점검: `research.json`(STEP1) → `post.md`/`post.html`(STEP2) → `image-prompts.json`(STEP3) → `metadata.json`/`guide.md`(STEP5).
> 3. **이미 완성된 STEP은 건너뛰고**, 누락·불완전한 STEP부터 이어서 진행합니다. 완성된 파일은 덮어쓰지 마세요(특히 post.md를 처음부터 다시 쓰지 말 것).
...
- **본문 분량: 공백 제외 3,500~4,000자.** 부족하면 섹션을 더 깊게(근거·예시·표·사례) 전개해 채우되 물타기 금지. 품질검사(STEP 4)에서 3,500자 미만이면 보강 후 재검사.
